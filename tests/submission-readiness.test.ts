...
test("the sealed submission package passes locally and exposes only authority-gated work", async () => {
  ...
  assert.throws(
    () => assertSubmissionRuntimeProvenance(submission.replace("Production version 13 at audited runtime commit", "Production version 12 at audited runtime commit")),
    /submission runtime provenance/,
  );
  assert.throws(
    () => assertSubmissionRuntimeProvenance(submission.replace("proof-producing version 7 runtime", "proof-producing version 13 runtime")),
    /submission runtime provenance/,
  );
  assert.throws(
  ...
  assert.throws(
    () => assertSubmissionRuntimeProvenance(submission.replace("appgdep_6a5c4df0a40c8191a791de2673f822ee", "appgdep_00000000000000000000000000000000")),
    /submission runtime provenance/,
  );
  assert.throws(
  ...
  assert.throws(
    () => assertYouTubeRuntimeProvenance(youtubeMetadata.replace("generated by version 7 runtime", "generated by version 13 runtime")),
    /YouTube runtime provenance/,
  );
  assert.throws(
    () => assertYouTubeRuntimeProvenance(youtubeMetadata.replace("fbb1153a831455513cc700cac0db0c5d40d2e59f", "042215042dd46ded14b501f961f4d9e7debb8178")),
    /YouTube runtime provenance/,
  );
  assert.throws(
    () => assertYouTubeRuntimeProvenance(`${youtubeMetadata}\nThe proof was generated by version 13 runtime \`fbb1153a831455513cc700cac0db0c5d40d2e59f\`.`),
  ...
  for (const key of Object.keys(releaseDocuments) as Array<keyof typeof releaseDocuments>) {
    const staleDocuments = { ...releaseDocuments, [key]: releaseDocuments[key].replace("version 13", "version 12") };
    ...
    assert.throws(
      ...
      /runtime provenance|stale v12-as-current claim/,
      `${key} must reject v12 as current`,
    );
  }
