...
test("the sealed submission package passes locally and exposes only authority-gated work", async () => {
  ...
  assert.throws(
    () => assertSubmissionRuntimeProvenance(submission.replace("Production version 15 at audited runtime commit", "Production version 14 at audited runtime commit")),
    /submission runtime provenance/,
  );
  assert.throws(
    () => assertSubmissionRuntimeProvenance(submission.replace("proof-producing version 7 runtime", "proof-producing version 15 runtime")),
    /submission runtime provenance/,
  );
  assert.throws(
  ...
  assert.throws(
    () => assertSubmissionRuntimeProvenance(submission.replace("appgdep_6a5c784ec5f081919b04d5c5fefd9191", "appgdep_00000000000000000000000000000000")),
    /submission runtime provenance/,
  );
  assert.throws(
  ...
  assert.throws(
    () => assertYouTubeRuntimeProvenance(youtubeMetadata.replace("generated by version 7 runtime", "generated by version 15 runtime")),
    /YouTube runtime provenance/,
  );
  assert.throws(
    () => assertYouTubeRuntimeProvenance(youtubeMetadata.replace("f823db6aaba4251a54839f4aa3be6cc67dbac451", "042215042dd46ded14b501f961f4d9e7debb8178")),
    /YouTube runtime provenance/,
  );
  assert.throws(
    () => assertYouTubeRuntimeProvenance(`${youtubeMetadata}\nThe proof was generated by version 15 runtime \`f823db6aaba4251a54839f4aa3be6cc67dbac451\`.`),
  ...
  for (const key of Object.keys(releaseDocuments) as Array<keyof typeof releaseDocuments>) {
    const staleDocuments = { ...releaseDocuments, [key]: releaseDocuments[key].replace("version 15", "version 14") };
    ...
    assert.throws(
      ...
      /runtime provenance|stale v14-as-current claim/,
      `${key} must reject v14 as current`,
    );
  }
