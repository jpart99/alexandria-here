...
Application and presentation lock: **COMPLETE on production Sites version 23.** The iExile/Mars Pathfinder sequence, persisted-only Timeline behavior, progressive Witness disclosure, summarized visitor warnings, cited title/Missing seams, desktop/mobile layout, dual live-proof contract, and zero-console production browser checks are accepted. The final dual-example version 23 media package is locally audited and integrity-pinned; public YouTube publication and Devpost synchronization remain pending.
...
Final demo video: **[ADD FINAL PUBLIC YOUTUBE URL — UNDER 3 MINUTES]**
...
Production version 23 at audited runtime commit `65b39285520cf871df1081d6899b083af3edbd83` passes 100 tests. The accepted Sites record is saved version `appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_bf2e23910c90819193de0e5adf1b785d`, deployment `appgdep_6a5d57fbccb08191b868daeaf3d8e0d8`, environment revision 7. It retains requested-year discovery, durable per-client cooldown enforcement, truthful decision provenance, exact Wayback replay identity, non-fabricated fallback labels, permanent legacy-receipt compatibility, witnessed browser-only font aliases, bounded streamed-body admission, exact JSON/HTML media-type checks, malformed-ID rejection before D1, private no-store recovery 404s, branded fail-closed persisted-result boundaries, refined invocation/Atlas typography, mobile containment, exact witnessed display-title selection, persisted-only Timeline inspection, progressive Witness disclosure, visitor-safe warning summarization, and cited title/Missing seams. It accepts bare, query-bearing, and archived `.onion` HTTP(S) locators through the same content-neutral safety boundary; it never contacts the submitted origin. For a query-bearing locator, discovery inventories the exact query plus the query-cleared sibling path across HTTP and HTTPS variants, merges and deduplicates surviving witnesses, and keeps the exact 12-record inventory and 8-capture fetch budgets. Receipt `1.3` gives that query-bearing identity an opaque full-SHA-256 route; receipts `1.0`–`1.2` permanently retain pathname-only relationship and replay semantics. The hosted production and live submission gates verified those contracts, pre-admission private-address rejection, exact font bytes, MIME, cache, `nosniff`, route markers, GET/HEAD/Range/ETag/304 behavior, lack of physical-path leakage, and both pinned presentation receipts.
...
The judging recovery above was generated by proof-producing version 7 runtime `042215042dd46ded14b501f961f4d9e7debb8178`; current production Sites version 23 serves and audits that persisted receipt 1.0 row but did not generate its GPT-5.6 decisions. Supporting witnesses, navigation, citations, and decision records were derived mechanically before validation.

Historical compatibility release v8 used runtime commit `f7f8f529285ed9e01fdbe02e868833fc86de5475`; its ordinary schema/model probe is recovery `6e467987-af60-4153-8d27-7653f56475aa`. The historical version-8 probe now fails closed under the current evidence-replay validator; its machine receipt is unavailable. Current v23 still preserves the documented pathname-only relationship and temporal-replay semantics for compatible receipts from `1.0` through `1.2`; it does not treat every historical packet as compatible.
...
## Final 2:31 demo provenance

The final master is `submission-assets/alexandria-here-build-week-demo.mp4`; its pinned SHA-256 is `ED270F6902CFA3AB96C0081E204A62670161E2E392DD470388306C9D57B3CA37`. It is 2:31.04 at 1920×1080 and 30 fps, with 4,531 frames, normalized 48 kHz synthetic narration, 56 exact English WebVTT cues, and real production interaction from both healthy presentation recoveries. The video includes no generated historical imagery or evidence. Its lineage includes [ordinary production recovery `8ea53a47-437b-4afe-ad2c-29c81637a327`](https://alexandria-here.cinemaexile.chatgpt.site/r/8ea53a47-437b-4afe-ad2c-29c81637a327). The historical video-capture row now fails closed under the current evidence-replay validator; its machine receipt is unavailable. It is retained only as lineage, not current proof.

The final Build Week video is **less than 3:00** and explains the working product, Codex's build role, and GPT-5.6's constrained runtime role in English.

**0:00–0:11 — The lost web.** Introduce the Library of Alexandria premise and the evidence-only promise.

**0:11–0:30 — What Alexandria returns.** Reveal the persisted iExile recovery.

**0:30–0:48 — iExile restored.** Browse the returned place and reveal block-level seams.

**0:48–1:10 — Recovery Atlas.** Show Timeline, Witnesses, What Survived, and the Receipt.

**1:10–1:36 — Mars Pathfinder.** Prove the same public pipeline on a second subject and era.

**1:36–2:02 — GPT-5.6 Chronologist.** Explain bounded model judgment and deterministic validation.

**2:02–2:20 — Codex and verification.** Show cited seams, absence, evidence windows, Witnesses, and Receipt.

**2:20–2:31 — Nothing without a witness.** Close on the evidence-only product promise.
...
- [x] Preserve the earlier public video as historical publication provenance.
- [x] Produce and independently audit the final version 23 video, captions, thumbnail, and six gallery cards as one sealed local set.
- [ ] Upload the locally audited final version 23 video, captions, and thumbnail as one sealed set.
...
- [x] Render and audit the final Devpost 3:2 project thumbnail and six ordered dual-example 3:2 gallery cards.
...
- [ ] Replace the saved Devpost About and judge instructions with the version 23 `DEVPOST_FIELD_COPY.md`, save, then verify Preview shows `100 passing tests`, the current runtime, and both presentation recoveries.
- [ ] Upload the final version 23 Devpost thumbnail and gallery media, then verify the public preview.
- [ ] Replace the sole bracketed final-video URL marker only after signed-out YouTube verification.
