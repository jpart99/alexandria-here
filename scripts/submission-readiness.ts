...
const CURRENT_RUNTIME_COMMIT = "f7f8f529285ed9e01fdbe02e868833fc86de5475";
const PROOF_RUNTIME_COMMIT = "042215042dd46ded14b501f961f4d9e7debb8178";
const CURRENT_RUNTIME_CLAIM = `Production version 8 at audited runtime commit \`${CURRENT_RUNTIME_COMMIT}\` passes 52 tests`;
const PROOF_RUNTIME_CLAIM = `The judging recovery above was generated by proof-producing version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; version 8 serves and audits that persisted row but did not generate its GPT-5.6 decisions.`;
const YOUTUBE_RUNTIME_CLAIM = `The current recovery and receipt links point to the corrected decision-attribution proof generated by version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; production version 8 runtime \`${CURRENT_RUNTIME_COMMIT}\` currently serves that persisted proof without claiming to have generated it.`;
...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  for (const claim of servingClaims) {
    ...
    if (version !== "8" || commit !== CURRENT_RUNTIME_COMMIT || (tests && tests !== "52")) {
...
export function assertSubmissionRuntimeProvenance(submission: string): void {
  requirePhrases(
    submission,
    [CURRENT_RUNTIME_CLAIM, PROOF_RUNTIME_CLAIM],
