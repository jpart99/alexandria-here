...
const CURRENT_RUNTIME_COMMIT = "3d786cb1f3c16e8a67e3c62d139f8f12f4986498";
...
const CURRENT_SITES_VERSION_ID = "appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_7a28c4a2c4688191b4dc099128059479";
const CURRENT_DEPLOYMENT_ID = "appgdep_6a5bfa7084748191abe03fa661ea8e3e";
const CURRENT_RUNTIME_CLAIM = `Production version 12 at audited runtime commit \`${CURRENT_RUNTIME_COMMIT}\` passes 66 tests`;
...
const PROOF_RUNTIME_CLAIM = `The judging recovery above was generated by proof-producing version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; current production Sites version 12 serves and audits that persisted row but did not generate its GPT-5.6 decisions.`;
...
const YOUTUBE_RUNTIME_CLAIM = `The current recovery and receipt links point to the corrected decision-attribution proof generated by version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; accepted production Sites version 12 runtime \`${CURRENT_RUNTIME_COMMIT}\` currently serves that persisted proof without claiming to have generated it.`;
...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  for (const claim of servingClaims) {
    ...
    if (version !== "12" || commit !== CURRENT_RUNTIME_COMMIT || (tests && tests !== "66")) {
