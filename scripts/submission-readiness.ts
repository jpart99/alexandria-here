...
const CURRENT_RUNTIME_COMMIT = "f823db6aaba4251a54839f4aa3be6cc67dbac451";
...
const CURRENT_SITES_VERSION_ID = "appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_56e85afb595c8191af9b0b7e48bdadda";
const CURRENT_DEPLOYMENT_ID = "appgdep_6a5c784ec5f081919b04d5c5fefd9191";
const CURRENT_RUNTIME_CLAIM = `Production version 15 at audited runtime commit \`${CURRENT_RUNTIME_COMMIT}\` passes 87 tests`;
...
const PROOF_RUNTIME_CLAIM = `The judging recovery above was generated by proof-producing version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; current production Sites version 15 serves and audits that persisted row but did not generate its GPT-5.6 decisions.`;
...
const YOUTUBE_RUNTIME_CLAIM = `The current recovery and receipt links point to the corrected decision-attribution proof generated by version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; accepted production Sites version 15 runtime \`${CURRENT_RUNTIME_COMMIT}\` currently serves that persisted proof without claiming to have generated it.`;
...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  for (const claim of servingClaims) {
    ...
    if (version !== "15" || commit !== CURRENT_RUNTIME_COMMIT || (tests && tests !== "87")) {
...
export function assertReleaseDocumentRuntimeProvenance(documents: ReleaseRuntimeDocuments): void {
  ...
  requirePhrases(
    documents.readme,
    [CURRENT_RUNTIME_CLAIM, CURRENT_SITES_RECORD_CLAIM, "Accepted production Sites version 15 serves that persisted proof without claiming that v15 generated it."],
    "README runtime provenance",
  );
  requirePhrases(
    documents.judgeEvidence,
    [
      `Current accepted release source (operator-recorded), production Sites version 15: [\`${CURRENT_RUNTIME_COMMIT}\`]`,
      `Saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7; 87 tests.`,
      "Production Sites version 15 serves this persisted version 7 proof",
      "split v15-serving/v7-proof provenance plus the v8 historical compatibility probe",
  ...
  requirePhrases(
    ...
    [
      `Production Sites version 15 was accepted only after the hosted gate passed against the public origin. Its audited source commit is \`${CURRENT_RUNTIME_COMMIT}\`; saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7.`,
      "Neither record should be relabeled as generated by v15.",
      WAYBACK_EXTERNAL_BLOCK_CLAIM,
    ],
