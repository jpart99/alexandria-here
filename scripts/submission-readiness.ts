...
const CURRENT_RUNTIME_COMMIT = "7c15b567d056ce13c6ef1bab4bf56aced3115cc0";
...
const CURRENT_SITES_VERSION_ID = "appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_29a5f9fa56248191adcc87a5bd6ec25c";
const CURRENT_DEPLOYMENT_ID = "appgdep_6a5c5de830b0819188e12f3f3a0b1ad4";
const CURRENT_RUNTIME_CLAIM = `Production version 14 at audited runtime commit \`${CURRENT_RUNTIME_COMMIT}\` passes 70 tests`;
...
const PROOF_RUNTIME_CLAIM = `The judging recovery above was generated by proof-producing version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; current production Sites version 14 serves and audits that persisted row but did not generate its GPT-5.6 decisions.`;
...
const YOUTUBE_RUNTIME_CLAIM = `The current recovery and receipt links point to the corrected decision-attribution proof generated by version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; accepted production Sites version 14 runtime \`${CURRENT_RUNTIME_COMMIT}\` currently serves that persisted proof without claiming to have generated it.`;
...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  for (const claim of servingClaims) {
    ...
    if (version !== "14" || commit !== CURRENT_RUNTIME_COMMIT || (tests && tests !== "70")) {
...
export function assertReleaseDocumentRuntimeProvenance(documents: ReleaseRuntimeDocuments): void {
  ...
  requirePhrases(
    documents.readme,
    [CURRENT_RUNTIME_CLAIM, CURRENT_SITES_RECORD_CLAIM, "Accepted production Sites version 14 serves that persisted proof without claiming that v14 generated it."],
    "README runtime provenance",
  );
  requirePhrases(
    documents.judgeEvidence,
    [
      `Current accepted release source (operator-recorded), production Sites version 14: [\`${CURRENT_RUNTIME_COMMIT}\`]`,
      `Saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7; 70 tests.`,
      "Production Sites version 14 serves this persisted version 7 proof",
      "split v14-serving/v7-proof provenance plus the v8 historical compatibility probe",
  ...
  requirePhrases(
    ...
    [
      `Production Sites version 14 was accepted only after the hosted gate passed against the public origin. Its audited source commit is \`${CURRENT_RUNTIME_COMMIT}\`; saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7.`,
      "Neither record should be relabeled as generated by v14.",
      WAYBACK_EXTERNAL_BLOCK_CLAIM,
    ],
  ...
  for (const [label, text] of Object.entries(documents)) {
    if (/\b(?:current|accepted|production Sites|Production version)[^\r\n]{0,160}\b(?:version\s+13|v13)\b/iu.test(text)) {
      fail(`${label} contains a stale v13-as-current claim`);
    }
  }
}
...
export async function runSubmissionReadiness(root = DEFAULT_ROOT): Promise<SubmissionCheck[]> {
  ...
  await addCheck(checks, "Submission contracts", "Evidence-backed submission narrative", async () => {
    ...
    return `v14 release documents, receipt, model, metric, Session ID, deadline, and ${youtubeState === "pending" ? "sole-placeholder" : "YouTube URL"} claims are present`;
  });
