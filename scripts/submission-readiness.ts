...
const CURRENT_RUNTIME_COMMIT = "88a4dce91b42a3fcc1d2adf9710de6bea651dfc4";
...
const CURRENT_SITES_VERSION_ID = "appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_d92c137f12788191bf5e69709b3809df";
const CURRENT_DEPLOYMENT_ID = "appgdep_6a5ccae8dcf48191b85e5a80613dc594";
...
const CURRENT_RUNTIME_CLAIM = `Production version 19 at audited runtime commit \`${CURRENT_RUNTIME_COMMIT}\` passes 96 tests`;
...
const PROOF_RUNTIME_CLAIM = `The judging recovery above was generated by proof-producing version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; current production Sites version 19 serves and audits that persisted receipt 1.0 row but did not generate its GPT-5.6 decisions.`;
...
const CURRENT_MATRIX_CLAIM = "Production version 19 passed the full eight-boundary compiled failure matrix.";
...
const DEVPOST_MEDIA_STATUS_CLAIM = "Status: authenticated Devpost About, judge instructions, project thumbnail, and six-image gallery synchronized and Preview-verified. The public YouTube video, rules acceptance, and final submission remain pending.";
const YOUTUBE_RUNTIME_CLAIM = `The current recovery and receipt links point to the corrected decision-attribution proof generated by version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; accepted production Sites version 19 runtime \`${CURRENT_RUNTIME_COMMIT}\` currently serves that persisted receipt 1.0 proof without claiming to have generated it.`;
...
const CHECKLIST = {
  ...
  devpostText: "Replace the saved Devpost About and judge instructions with `DEVPOST_FIELD_COPY.md`, save, then verify Preview shows `96 passing tests` and the current judging recovery.",
...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  for (const claim of servingClaims) {
    ...
    if (version !== "19" || commit !== CURRENT_RUNTIME_COMMIT || (tests && tests !== "96")) {
  ...
  if (/\b(?:current|accepted)\s+(?:(?:production\s+Sites)|(?:Sites\s+production))\s+version\s+(?:14|16|17|18)\b/i.test(text)) {
    fail(`${label} contains a stale pre-v19 current-runtime claim`);
  }
}
...
export function assertReleaseDocumentRuntimeProvenance(documents: ReleaseRuntimeDocuments): void {
  ...
  requirePhrases(
    ...
    [
      CURRENT_RUNTIME_CLAIM,
      CURRENT_SITES_RECORD_CLAIM,
      "Accepted production Sites version 19 serves that persisted receipt 1.0 proof without claiming that v19 generated it.",
      HISTORICAL_V16_RELEASE_CLAIM,
      HISTORICAL_V17_RELEASE_CLAIM,
      HISTORICAL_V18_RELEASE_CLAIM,
      IEXILE_RESCUE_CLAIM,
  ...
  requirePhrases(
    ...
    [
      `Current accepted release source (operator-recorded), production Sites version 19: [\`${CURRENT_RUNTIME_COMMIT}\`]`,
      `Saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7; 96 tests.`,
      "Production Sites version 19 serves this persisted version 7 receipt 1.0 proof",
      "split v19-serving/v7-proof provenance plus the v8 historical compatibility probe",
      HISTORICAL_V16_RELEASE_CLAIM,
      HISTORICAL_V17_RELEASE_CLAIM,
      HISTORICAL_V18_RELEASE_CLAIM,
      IEXILE_RESCUE_CLAIM,
  ...
  requirePhrases(
    ...
    [
      `Production Sites version 19 was accepted only after the hosted gate passed against the public origin. Its audited source commit is \`${CURRENT_RUNTIME_COMMIT}\`; saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7.`,
      "Neither record should be relabeled as generated by v19.",
      HISTORICAL_V16_RELEASE_CLAIM,
      HISTORICAL_V17_RELEASE_CLAIM,
      HISTORICAL_V18_RELEASE_CLAIM,
      IEXILE_RESCUE_CLAIM,
  ...
  for (const [label, text] of Object.entries(documents)) {
    ...
    if (/\b(?:current|accepted|production Sites|Production version)[^\r\n]{0,160}\b(?:version\s+(?:14|16|17)|v(?:14|16|17))\b/iu.test(text)) {
      fail(`${label} contains a stale pre-v19-as-current claim`);
    }
  }
}
...
export async function runSubmissionReadiness(root = DEFAULT_ROOT): Promise<SubmissionCheck[]> {
  ...
  await addCheck(checks, "Submission contracts", "Devpost handoff", async () => {
    ...
    requirePhrases(handoff, [VIDEO_NAME, VIDEO_HASH, YOUTUBE_THUMBNAIL_NAME, CAPTIONS_NAME, DEVPOST_NAMES[0], "devpost-media.sha256", "DEVPOST_FIELD_COPY.md", "project thumbnail, and six-card gallery are saved and Preview-verified", "Authenticated synchronization on July 19, 2026", "Preview was then verified", "96 passing tests", RECOVERY_ID, "3/5 steps done", "project thumbnail is the exact audited `08-devpost-cover.png`", "image gallery contains exactly six audited cards in numbered order", "gallery tabs `1 of 6` through `6 of 6`", "video-demo field is empty", "official-rules checkbox is unchecked", "Do not accept the official rules or press **Submit project** during synchronization.", "less than 3:00", "2:35.26", "July 21, 2026 at 5:00 PM PDT (Pacific Time)", "https://openai.devpost.com/rules", "https://openai.devpost.com/details/faqs", PRODUCTION_URL, REPOSITORY_URL, RECOVERY_URL, RECEIPT_URL, VIDEO_CAPTURE_RECOVERY_URL, SESSION_ID, "up to 15 images", "5 MB", "Jaia's authority", "Devpost media synchronization is complete", "public YouTube publication", "official-rules acceptance", "final submission", EXAMPLE_SCOPE_CLAIM], "final handoff");
  ...
  await addCheck(checks, "Submission contracts", "Evidence-backed submission narrative", async () => {
    ...
    return `v19 release documents, receipt, model, metric, Session ID, deadline, and ${youtubeState === "pending" ? "sole-placeholder" : "YouTube URL"} claims are present`;
  });
