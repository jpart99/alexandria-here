...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  if (/\bgenerated by (?:the )?(?:(?:current )?production |proof-producing )?version 18\b/i.test(text)) {
    fail(`${label} claims production version 18 generated the persisted judging proof`);
  }
}
