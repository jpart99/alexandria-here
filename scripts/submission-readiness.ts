...
const CURRENT_RUNTIME_COMMIT = "d32ab887e880d7f3d4bbf1c9d71e0aec37388a43";
...
const CURRENT_SITES_VERSION_ID = "appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_76f67dace6088191b2b415d5e4b1d17b";
const CURRENT_DEPLOYMENT_ID = "appgdep_6a5c90b3020c81919c73b5a84e39580e";
const CURRENT_RUNTIME_CLAIM = `Production version 16 at audited runtime commit \`${CURRENT_RUNTIME_COMMIT}\` passes 91 tests`;
...
const PROOF_RUNTIME_CLAIM = `The judging recovery above was generated by proof-producing version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; current production Sites version 16 serves and audits that persisted receipt 1.0 row but did not generate its GPT-5.6 decisions.`;
...
const YOUTUBE_RUNTIME_CLAIM = `The current recovery and receipt links point to the corrected decision-attribution proof generated by version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; accepted production Sites version 16 runtime \`${CURRENT_RUNTIME_COMMIT}\` currently serves that persisted receipt 1.0 proof without claiming to have generated it.`;
const WAYBACK_EXTERNAL_BLOCK_CLAIM = "The live CDX portion of the eight-scenario failure matrix is externally blocked because public Wayback CDX returned zero bytes or timed out";
...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  for (const claim of servingClaims) {
    ...
    if (version !== "16" || commit !== CURRENT_RUNTIME_COMMIT || (tests && tests !== "91")) {
...
export function assertSubmissionRuntimeProvenance(submission: string): void {
  requirePhrases(
    submission,
    [CURRENT_RUNTIME_CLAIM, CURRENT_SITES_RECORD_CLAIM, PROOF_RUNTIME_CLAIM, HISTORICAL_COMPATIBILITY_CLAIM, WAYBACK_EXTERNAL_BLOCK_CLAIM],
...
export function assertReleaseDocumentRuntimeProvenance(documents: ReleaseRuntimeDocuments): void {
  ...
  requirePhrases(
    documents.readme,
    [CURRENT_RUNTIME_CLAIM, CURRENT_SITES_RECORD_CLAIM, "Accepted production Sites version 16 serves that persisted receipt 1.0 proof without claiming that v16 generated it."],
    "README runtime provenance",
  );
  requirePhrases(
    documents.judgeEvidence,
    [
      `Current accepted release source (operator-recorded), production Sites version 16: [\`${CURRENT_RUNTIME_COMMIT}\`]`,
      `Saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7; 91 tests.`,
      "Production Sites version 16 serves this persisted version 7 receipt 1.0 proof",
      "split v16-serving/v7-proof provenance plus the v8 historical compatibility probe",
      WAYBACK_EXTERNAL_BLOCK_CLAIM,
    ],
  ...
  requirePhrases(
    ...
    [
      `Production Sites version 16 was accepted only after the hosted gate passed against the public origin. Its audited source commit is \`${CURRENT_RUNTIME_COMMIT}\`; saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7.`,
      "Neither record should be relabeled as generated by v16.",
      WAYBACK_EXTERNAL_BLOCK_CLAIM,
    ],
  ...
  for (const [label, text] of Object.entries(documents)) {
    if (/\b(?:current|accepted|production Sites|Production version)[^\r\n]{0,160}\b(?:version\s+14|v14)\b/iu.test(text)) {
      fail(`${label} contains a stale v14-as-current claim`);
    }
  }
}
...
export async function runSubmissionReadiness(root = DEFAULT_ROOT): Promise<SubmissionCheck[]> {
  ...
  await addCheck(checks, "Submission contracts", "Evidence-backed submission narrative", async () => {
    const [submission, readme, judgeEvidence, releaseOperations, youtubeMetadata] = await Promise.all([
