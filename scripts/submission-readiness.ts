...
const CURRENT_RUNTIME_COMMIT = "fbb1153a831455513cc700cac0db0c5d40d2e59f";
...
const CURRENT_SITES_VERSION_ID = "appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_11f255a52e588191b76307da320fbe10";
const CURRENT_DEPLOYMENT_ID = "appgdep_6a5c4df0a40c8191a791de2673f822ee";
const CURRENT_RUNTIME_CLAIM = `Production version 13 at audited runtime commit \`${CURRENT_RUNTIME_COMMIT}\` passes 66 tests`;
...
const PROOF_RUNTIME_CLAIM = `The judging recovery above was generated by proof-producing version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; current production Sites version 13 serves and audits that persisted row but did not generate its GPT-5.6 decisions.`;
...
const YOUTUBE_RUNTIME_CLAIM = `The current recovery and receipt links point to the corrected decision-attribution proof generated by version 7 runtime \`${PROOF_RUNTIME_COMMIT}\`; accepted production Sites version 13 runtime \`${CURRENT_RUNTIME_COMMIT}\` currently serves that persisted proof without claiming to have generated it.`;
...
function assertExclusiveRuntimeClaims(text: string, label: string): void {
  ...
  for (const claim of servingClaims) {
    ...
    if (version !== "13" || commit !== CURRENT_RUNTIME_COMMIT || (tests && tests !== "66")) {
...
export function assertSubmissionRuntimeProvenance(submission: string): void {
  requirePhrases(
    submission,
    [CURRENT_RUNTIME_CLAIM, CURRENT_SITES_RECORD_CLAIM, PROOF_RUNTIME_CLAIM, HISTORICAL_COMPATIBILITY_CLAIM],
...
export function assertReleaseDocumentRuntimeProvenance(documents: ReleaseRuntimeDocuments): void {
  ...
  requirePhrases(
    documents.readme,
    [CURRENT_RUNTIME_CLAIM, CURRENT_SITES_RECORD_CLAIM, "Accepted production Sites version 13 serves that persisted proof without claiming that v13 generated it."],
    "README runtime provenance",
  );
  requirePhrases(
    documents.judgeEvidence,
    [
      `Current accepted release source (operator-recorded), production Sites version 13: [\`${CURRENT_RUNTIME_COMMIT}\`]`,
      `Saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7; 66 tests.`,
      "Production Sites version 13 serves this persisted version 7 proof",
      "split v13-serving/v7-proof provenance plus the v8 historical compatibility probe",
    ],
  ...
  requirePhrases(
    ...
    [
      `Production Sites version 13 was accepted only after the hosted gate passed against the public origin. Its audited source commit is \`${CURRENT_RUNTIME_COMMIT}\`; saved version \`${CURRENT_SITES_VERSION_ID}\`; deployment \`${CURRENT_DEPLOYMENT_ID}\`; environment revision 7.`,
      "Neither record should be relabeled as generated by v13.",
    ],
    "release operations runtime provenance",
  );

  for (const [label, text] of Object.entries(documents)) {
    if (/\b(?:current|accepted|production Sites|Production version)[^\r\n]{0,160}\b(?:version\s+12|v12)\b/iu.test(text)) {
      fail(`${label} contains a stale v12-as-current claim`);
    }
  }
}
...
export async function runSubmissionReadiness(root = DEFAULT_ROOT): Promise<SubmissionCheck[]> {
  ...
  await addCheck(checks, "Submission contracts", "Evidence-backed submission narrative", async () => {
    ...
    return `v13 release documents, receipt, model, metric, Session ID, deadline, and ${youtubeState === "pending" ? "sole-placeholder" : "YouTube URL"} claims are present`;
  });
