...
Run `npm run qa:submission:live` immediately after final submission and at least once per day through the judging deadline. Treat any failure as a release incident. Restore the exact public surface and rerun the gate; never recreate or relabel the persisted proof row merely to make a check pass.

## Accepted production record

Production Sites version 16 was accepted only after the hosted gate passed against the public origin. Its audited source commit is `d32ab887e880d7f3d4bbf1c9d71e0aec37388a43`; saved version `appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_76f67dace6088191b2b415d5e4b1d17b`; deployment `appgdep_6a5c90b3020c81919c73b5a84e39580e`; environment revision 7. The source gate passed 91 tests; TypeScript, lint, the clean production build, dependency audit, compiled release gate, hosted production gate, and live submission proof also passed. The hosted check proved the content-neutral bare/query-bearing/archived-`.onion` HTTP(S) form contract, pre-admission rejection of a bare private address, exact browser-only alias discovery, no physical `/fonts/` namespace leakage, exact font bytes and headers, GET/HEAD/Range/ETag/304 behavior, ordinary asset bypass, safe missing-alias handling, unsafe-URL rejection, fail-closed receipt lookup, and exact agreement with persisted reference recovery `18026989-33be-4011-86ee-19e1754cb22c`. Unit gates separately prove that query discovery inventories the exact query plus its query-cleared sibling path across HTTP and HTTPS variants, merges and deduplicates witnesses within the 12-record/8-fetch budgets, assigns opaque full-SHA-256 route identity in receipt `1.3`, and permanently preserves pathname-only relationship and replay semantics for receipts `1.0`–`1.2`. The exact judging row remains receipt `1.0`; v16 serves and audits it without rewriting or reattributing it.

The proof recovery remains attributed to version 7 runtime `042215042dd46ded14b501f961f4d9e7debb8178`. Historical compatibility release v8 used runtime commit `f7f8f529285ed9e01fdbe02e868833fc86de5475`; ordinary probe `6e467987-af60-4153-8d27-7653f56475aa` records that release's native receipt/model path. Neither record should be relabeled as generated by v16.
