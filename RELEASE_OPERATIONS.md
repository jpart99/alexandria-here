...
Production Sites version 19 was accepted only after the hosted gate passed against the public origin. Its audited source commit is `88a4dce91b42a3fcc1d2adf9710de6bea651dfc4`; saved version `appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_d92c137f12788191bf5e69709b3809df`; deployment `appgdep_6a5ccae8dcf48191b85e5a80613dc594`; environment revision 7. The source gate passed 96 tests; TypeScript, lint, the clean production build, dependency audit, compiled release gate, hosted production gate, full eight-boundary compiled failure matrix, live submission proof, exact JSON/HTML media-type rejection, and bounded streamed-body admission also passed. The hosted check proved the content-neutral bare/query-bearing/archived-`.onion` HTTP(S) form contract, pre-admission rejection of a bare private address, exact browser-only alias discovery, no physical `/fonts/` namespace leakage, exact font bytes and headers, GET/HEAD/Range/ETag/304 behavior, ordinary asset bypass, branded missing-result boundaries, unsafe-URL rejection, fail-closed receipt lookup, and exact agreement with persisted reference recovery `18026989-33be-4011-86ee-19e1754cb22c`. Unit gates separately prove that query discovery inventories the exact query plus its query-cleared sibling path across HTTP and HTTPS variants, merges and deduplicates witnesses within the 12-record/8-fetch budgets, assigns opaque full-SHA-256 route identity in receipt `1.3`, and permanently preserves pathname-only relationship and replay semantics for receipts `1.0`–`1.2`. The exact judging row remains receipt `1.0`; v19 serves and audits it without rewriting or reattributing it.
