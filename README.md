...
The sealed master shows ordinary production recovery `8ea53a47-437b-4afe-ad2c-29c81637a327`; its planner, model, and count claims remain receipt-backed. Current judging links use corrected recovery `18026989-33be-4011-86ee-19e1754cb22c`, generated by the proof-producing version 7 runtime `042215042dd46ded14b501f961f4d9e7debb8178`, whose decision attribution separates deterministic era selection from GPT-5.6 page-order and primary-witness decisions. Production version 8 serves that persisted proof without claiming that v8 generated it.
...
The release candidate exposes two exact browser-facing aliases under `/witness-fonts/`. Those aliases deliberately have no static-file match, so Sites falls through to the Worker; the Worker rewrites each alias to its exact physical `/fonts/` asset through the managed `ASSETS` binding and adds `font/woff2`, `nosniff`, bounded public caching, and route-proof headers. HTML, CSS, and preloads never expose the physical paths, while all ordinary compiled assets remain asset-first. `public/_headers` is retained only as a portable physical-store fallback; it is not treated as proof of Sites production behavior. This paragraph becomes a production claim only after the hosted smoke passes on the deployed version.
...
Production version 8 runs audited runtime commit `f7f8f529285ed9e01fdbe02e868833fc86de5475`. The release passes 52/52 tests, TypeScript, lint, a clean production build, public browser regression, and a zero-vulnerability production dependency audit. Version 8 adds receipt capture metadata and exact warning groups with source/capture/block ownership. For legacy rows it derives those additive fields from the persisted recovery on read, without rewriting managed D1 or relabeling the legacy receipt version; the downloaded JSON presentation may therefore change while the persisted recovery ID, manifest hash, source hashes, decisions, and validations remain unchanged.

The ordinary public pipeline on proof-generating version 7 runtime `042215042dd46ded14b501f961f4d9e7debb8178` produced the [receipt-backed GPT-5.6 iExile recovery](https://alexandria-here.cinemaexile.chatgpt.site/r/18026989-33be-4011-86ee-19e1754cb22c): 5 returned preserved pages plus 2 witnessed Missing states from 8 capture records, 347 rendered blocks, 946 content-addressed extracted evidence blocks, 36 inferred edges, 8 known absences, and all 10 receipt validations passing. Its corrected decisions attribute the era to deterministic selection and exactly two decision kinds to GPT-5.6: page order and primary-witness choice. Production version 8 presents and audits this persisted row; it is not the runtime that generated the proof.
