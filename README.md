...
Production Sites version 23 ships content-neutral query-bearing and archived `.onion` HTTP(S) locator support, receipt `1.3` query identity, permanent receipt `1.0`–`1.2` pathname-only compatibility, bounded streamed-body admission, exact media-type checks, malformed-ID rejection before D1, private no-store recovery 404s, a fail-closed persisted-result boundary, refined invocation/Atlas typography, mobile containment, exact witnessed display-title selection, persisted-only Timeline inspection, progressive Witness disclosure, summarized visitor warnings, and cited title/Missing seams. Its saved version deployed successfully and its hosted production and live submission gates passed; the exact judging row remains receipt `1.0` and is not presented as evidence generated by v23.
