...
Production Sites version 28 ships the full content-neutral recovery engine plus the post-deadline conservation-lab interface: an invocation-first light landing, light Recovery Atlas chrome, a receipt-backed evidence ribbon, explicit normalized-reading disclosure, restrained mobile typography, distinct successful/insufficient ribbon language, and the explicit preservation-is-not-endorsement doctrine. Its saved version deployed successfully and its hosted desktop/mobile/browser gates passed. A fresh bare-host `iexile.com` recovery created ordinary ID `705709f4-e9c8-4037-8c5e-c78d4857341e`, rendered a first-class `insufficient_evidence` Atlas instead of raw `Not Found`, and returned a receipt `1.3` GPT-5.6 record. The exact judging row remains receipt `1.0` and is not presented as evidence generated by v28.
...
Current deployed version 28 uses audited runtime commit `486d07a3236670c05f9af5143755419d8647fae0` and passes 100 tests. Its Sites record is saved version `appgprj_6a5b165146ec8191a6507491526ca6eb~appgver_cf394d7d08608191b45959acc939b5de`, deployment `appgdep_6a60bc79c1e88191816e07e8379b661f`, environment revision 7. It also passes TypeScript, zero-warning lint, a clean production build, `qa:release`, the live `qa:production` contract, a zero-vulnerability production dependency audit, desktop and exact 320 px browser containment, zero-console landing/restored/insufficient states, all five Atlas panels, Show the Seams, the persisted-data ribbon, receipt download, and a fresh ordinary bare-host recovery. Receipt `1.3` gives every query-bearing locator an opaque full-SHA-256 route identity while receipts `1.0`–`1.2` permanently retain their pathname-only relationship and replay semantics. The full eight-boundary compiled failure matrix remains inherited from accepted version 23 and was not re-run against v28.
